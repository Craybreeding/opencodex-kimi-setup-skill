...
18. Keep the `moonshot` provider (Moonshot Open Platform, pay-per-token balance) distinct from `kimi-code` (Kimi Code subscription quota). Prefer `https://api.moonshot.ai/v1` over `api.moonshot.cn`: the `.cn` hostname can resolve to Clash fake-IP (`198.18.0.0/15`) and be blocked by destination policy even with `allowPrivateNetwork` on, while `.ai` resolves publicly. Do not add provider hosts to `NO_PROXY` to work around fake-IP. Exception: domestic-platform keys (platform.moonshot.cn) are a separate account system and hard-401 on `.ai`; with such a key the registry `.cn` pin is mandatory (see rule 21).
19. An unresolved `${ENV_VAR}` `apiKey` reference is a latent outage: any process started without that variable (launchd, a watchdog, a scheduler) fails every request with `openai-chat requires a non-empty credential (authMode: key)`. Prefer an embedded, upstream-validated key in `apiKey`, and prune dead entries from `apiKeyPool` so key rotation can never land on an env reference or an expired key. Verify each pool key directly against the provider endpoint before trusting it; never print the key while doing so.
20. Treat the port listener's owner as part of health: a PID on 10100 is not proof the managed service is running. Confirm `launchctl print gui/<uid>/com.opencodex.proxy` (or the equivalent supervisor) owns the listener; watchdog-spawned instances can run with a degraded environment and log to a different file, hiding real errors from `service.log`.
21. After any `npm` upgrade of the opencodex package, search the source for `Local patch` markers and re-apply them before restart: the Moonshot registry baseUrl pin (`.ai` vs `.cn` accounts are independent), the Kimi model-identity injection, and the DeepSeek null-type tool-schema sanitize. Upstream 2.10.x already ships K3 model metadata and `preserveReasoningContentModels` natively, but still pins Moonshot to the international endpoint. Also note the machine's npm prefix may not be what `which npm` implies; upgrade with an explicit `--prefix` targeting the install the service actually runs.
